...
def clients_global_summary():
    ...
    limits = {'RMA10': {'vis': 10.44, 'den': 0.9209},
              ...
              'RMF180': {'vis': 187.9, 'den': 0.9919},
              'RMG180': {'vis': 187.9, 'den': 0.9919},
              'RMG380': {'vis': 396.6, 'den': 0.9919},
              'RMG500': {'vis': 521.8, 'den': 0.9919},
              'RMH380': {'vis': 396.6, 'den': 0.9919},
              'RMH500': {'vis': 521.8, 'den': 0.9919},
              'RMH700': {'vis': 730.6, 'den': 0.9919},
              'RMK380': {'vis': 396.6, 'den': 1.0109},
              'RMK500': {'vis': 521.8, 'den': 1.0109},
              'RMK700': {'vis': 730.6, 'den': 1.0109},
              }

    df2 = df2[(df2['adv_density'] != 'N/S') | (df2['adv_density'] != 'N/A')]
    df3 = df2[["adv_density", "TESTED_DEN",
               "advised grade", 'material_type']].reset_index(drop=True)
    len_den = len(df2[df2['material_type'] == 'HFO'])
    for l in limits:
        den = df3[(df3['advised grade'] == l) &
                  (df3['TESTED_DEN'].astype(float) <= limits[l]['den']) &
                  (df3['material_type'] == 'HFO')]
    ...
    df4 = df2[["adv_vis", "V50_Result",
               "advised grade"]].reset_index(drop=True)
    for l in limits:
        vis = df4[(df4['advised grade'] == l) &
                  (df4['V50_Result'].astype(float) <= limits[l]['vis'])]
...
def clients_summary(cl):
    ...
    df2 = df2[df2['clients'].str.contains(cl, case=False)]

    if len(df2[df2['overall_lr_outcome'] == 'RED']) > 0:
        perc_red = 100 - (len(df2[df2['overall_lr_outcome'] == 'RED']) / len(df2)) * 100
    else:
        perc_red = 100
    ...
    df3 = df2[["adv_density", "TESTED_DEN",
               "advised grade", 'material_type']].reset_index(drop=True)
    len_den = len(df3[df3['material_type'] == 'HFO'])
    for l in limits:
        den = df3[(df3['advised grade'] == l) &
                  (df3['TESTED_DEN'].astype(float) <= limits[l]['den']) &
                  (df3['material_type'] == 'HFO')]
    ...
    df2 = df2[df2['adv_vis'] != 'N/S']
    df2 = df2[df2['advised grade'].str.contains('R')]
    df4 = df2[["adv_vis", "V50_Result",
               "advised grade"]].reset_index(drop=True)
    for l in limits:
        vis = df4[(df4['advised grade'] == l) &
...
client_stats = clients_summary(shared['cl'])
...
plt.yticks([60, 70, 80, 90, 100], ["60%", "70%", "80%", "90%", "100%"],
           color='#284d97', size=16, fontname='DejaVu Sans')
plt.ylim(60, 100)
...
ax.plot(angles, values, linewidth=1, linestyle='solid',
        label="Global", color='orange', zorder=-5)
ax.fill(angles, values, 'orange', alpha=0.1, zorder=-5)
ax.yaxis.set_visible(True)
plt.xticks(angles[:-1], categories, color='#414042',
           fontname='DejaVu Sans', size=16, zorder=5)
values = df.loc[1].drop('group').values.flatten().tolist()
values += values[:1]
print(values)
ax.plot(angles, values, linewidth=1, linestyle='solid',
        label="Client", color='#3B8EDE', zorder=-5)
ax.fill(angles, values, '#3B8EDE', alpha=0.1, zorder=-5)
ax.set_frame_on(True)
font = font_manager.FontProperties(family='DejaVu Sans',
                                   weight='normal',
                                   style='normal', size=16)

client_leg_patch = mpatches.Patch(color='orange', label='Global')
global_leg_patch = mpatches.Patch(color='#3B8EDE', label=shared['cl'].title())
leg = plt.legend(handles=[client_leg_patch, global_leg_patch],
                 loc='upper left', bbox_to_anchor=(-0.1, 1.05), prop=font, framealpha=0.5)
