with
  samples as (SELECT "samples".*, case when attachments.file_file_name ~* 'bdr|bdn' then 'yes' else 'no' end as "BDN" FROM "samples" 
  LEFT OUTER JOIN "clients" ON "clients"."id" = "samples"."client_id" 
  join attachments on samples.id = attachments.parent_id 
  WHERE "samples"."client_id" IN (SELECT "clients"."id" FROM "clients" 
  WHERE (clients.id != 1583)) AND ("samples"."validated_on"::date >= '{df}' AND "samples"."validated_on"::date <= '{dt}'))
select distinct 
  samples.id,
  samples."BDN",
