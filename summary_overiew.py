...
def clients_summary(cl):
    ...
    print(df3[df3['material_type'] == 'HFO'])
    for l in limits:
        den = df3[(df3['advised grade'] == l) & (df3['TESTED_DEN'].astype(float) <= limits[l]['den']) & (df3['material_type'] == 'HFO')]        
        dc.append(den['TESTED_DEN'].tolist())
    if len(dc) > 0:
...
plt.xticks(angles[:-1], categories, color='#414042', fontname='DejaVu Sans', size=16, )

ax.set_rlabel_position(0)
plt.yticks([60, 70, 80, 90, 100], ["60", "70", "80", "90", "100"], color='#414042', size=16, fontname='DejaVu Sans')
plt.ylim(60, 100)

values = df.loc[0].drop('group').values.flatten().tolist()
values += values[:1]
print(values)
ax.plot(angles, values, linewidth=1, linestyle='solid', label="Global", color='orange')
ax.fill(angles, values, 'orange', alpha=0.1)
ax.yaxis.set_visible(True)
values = df.loc[1].drop('group').values.flatten().tolist()
values += values[:1]
print(values)
ax.plot(angles, values, linewidth=1, linestyle='solid', label="Client", color='#3B8EDE')
...
leg = plt.legend(handles=[client_leg_patch, global_leg_patch], loc='upper left', bbox_to_anchor=(-0.1, 1.05), prop=font)
